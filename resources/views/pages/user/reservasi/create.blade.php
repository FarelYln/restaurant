...
@endif
    <form id="reservationForm" action="{{ route('user.reservasi.store') }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="date" name="tanggal_reservasi" class="form-control @error('tanggal_reservasi') is-invalid @enderror" 
        ...
        <div class="form-group mt-3">
            ...
            <select name="id_meja" class="form-control @error('id_meja') is-invalid @enderror" required>
                ...
                @foreach ($meja as $m)
                    <option value="{{ $m->id }}" {{ old('id_meja') == $m->id ? 'selected' : '' }}>{{ $m->nomor_meja }}</option>
                @endforeach
            </select>
            @error('id_meja')
                <small class="text-danger">{{ $message }}</small>
        ...
        <div class="form-group mt-3">
            ...
            <div class="d-flex flex-wrap overflow-auto" style="max-width: 100%; padding: 10px;">
                @foreach ($menus as $menu)
                    <div class="card" style="width: 150px; margin: 10px; cursor: pointer;" id="menu-card-{{ $menu->id }}" onclick="toggleMenuSelection(event, {{ $menu->id }})">
                        <button type="button" class="btn btn-danger btn-sm" id="close-{{ $menu->id }}" style="position: absolute; top: 10px; left: 10px; z-index: 2;" onclick="closeMenu({{ $menu->id }}); event.stopPropagation();">-</button>
                        <img src="{{ asset('storage/' . $menu->image) }}" class="card-img-top" alt="{{ $menu->name }}" style="height: 120px; object-fit: cover;">
                        <div class="card-body">
                            <h5 class="card-title text-center">{{ $menu->name }}</h5>
                            <p class="card-text text-center">Rp {{ number_format($menu->harga, 0, ',', '.') }}</p>
                            <div class="d-flex justify-content-between align-items-center">
                                <button type="button" class="btn btn-secondary btn-sm" id="decrease-{{ $menu->id }}" style="display: none;" onclick="adjustQuantity({{ $menu->id }}, -1)">-</button>
                                <input type="number" class="form-control form-control-sm text-center" id="quantity-{{ $menu->id }}" value="0" style="width: 50px; display: none;" onchange="updateQuantity({{ $menu->id }})">
                                <button type="button" class="btn btn-primary btn-sm" id="increase-{{ $menu->id }}" style="display: none;" onclick="adjustQuantity({{ $menu->id }}, 1)">+</button>
                            </div>
                        </div>
                    </div>
                    <input type="hidden" name="menu[{{ $menu->id }}]" value="0" id="menu-input-{{ $menu->id }}">
                @endforeach
            </div>
        </div>
...
function showConfirmationModal() {
    ...
    const table = document.querySelector('[name="id_meja"]').options[document.querySelector('[name="id_meja"]').selectedIndex].text;
